...
<script>
function go(zipcode,no,sido,gugun,dong,bunji){
	var sum = no+sido+gugun+dong+bunji;
	opener.document.form_name.zipcode.value=zipcode;
	opener.document.form_name.addr1.value=sum
	window.close();
}
...
<TABLE CELLPADDING=0 CELLSPACING=0 BORDER=0 WIDTH=330><TR><TD BGCOLOR=#948DCF>
  <TABLE CELLPADDING=0 CELLSPACING=1 BORDER=0 WIDTH=330><TR><TD>
    ...
    <FORM NAME="inquiry" METHOD="post" ACTION="postCheck.do" onSubmit="return check();">
      <TR>
        <TD ALIGN=CENTER><br>
      ...
      <TR>
        <TD><a href="#" onClick="go('${zipcode.zipcode}','${zipcode.no}','${zipcode.sido}','${zipcode.gugun}','${zipcode.dong}','${zipcode.bunji}')">
        ${zipcode.zipcode} ${zipcode.no} ${zipcode.sido} ${zipcode.gugun} ${zipcode.dong} ${zipcode.bunji}
        </a>>
        </TD>
      </TR>
      </c:forEach>
