...
<TABLE CELLPADDING=0 CELLSPACING=0 BORDER=0 WIDTH=330><TR><TD BGCOLOR=#948DCF>
  <TABLE CELLPADDING=0 CELLSPACING=1 BORDER=0 WIDTH=330><TR><TD>
    ...
    <FORM NAME="pform" METHOD="post" ACTION="post_check.jsp" onSubmit="return check();">
      <TR>
        <TD ALIGN=CENTER><br>
...
<%
    Set set = zip.entrySet();//키와 값이 한쌍으로 리턴됨 
	Iterator iter = set.iterator(); //반복자
	while(iter.hasNext()){ //존재할때 까지
		Map.Entry entry = (Map.Entry)iter.next();
		if(entry.getValue().toString().contains(addr)){ //문자열로 변환후 검색
%>

        <TD><a href="#" onClick="javascript:post_send('<%=entry.getKey()%>','<%=entry.getValue() %>')"><%=entry.getKey()%>-<%=entry.getValue() %></a>
        </TD>
      </TR>
<%
        }
