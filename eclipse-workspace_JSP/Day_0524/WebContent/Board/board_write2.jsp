<%@ page import="java.util.*" import="Day_0524_board.*" contentType="text/html; charset=UTF-8" %>

<%!List<boardBean> list = new ArrayList<boardBean>();%>
<html>
   <head><title>게시판 작성</title>
    <link rel="stylesheet" type="text/css" href="/stylesheet.css">    
<jsp:useBean id="board" class="Day_0524_board.boardBean">
<jsp:setProperty name="board" property="*"/>
</jsp:useBean>
<%
String go = request.getParameter("h");
if(go!=null){
	if(go.equals("go")){
		request.setAttribute("list", list);
		RequestDispatcher rd = request.getRequestDispatcher("board_list.jsp");
		rd.forward(request, response);
	}
}
%>
<%
if(board.getName()!=null){
if(list.add(board)){
%>
<script>
alert("등록성공!");
</script>
<%}
}%>


<script>
function board_send(){
bform.action="board_write2.jsp";
bform.submit();
}
function board_list(){	
bform.h.value="go";
bform.action="#";
bform.submit();
}
</script>
</head>
 <body topmargin="0" leftmargin="0">
 <table border="0" width="800">
 <tr>
   <td width="20%" height="500" bgcolor="#ecf1ef" valign="top">

   <!-- 다음에 추가할 부분 -->

   </td>

   <td width="80%" valign="top">&nbsp;<br>
     <img src="./img/bullet-01.gif"><font size="3" face="돋움" color="blue"> <b>반갑습니다</b></font>
     <font size="2"> - 글쓰기</font><p>
     <img src="./img/bullet-03.gif"><font size="2" face="돋움" color="orange"> 잠깐</font> &nbsp;
     <img src="./img/bullet-02.gif"><font size="2" face="돋움">는 필수 입력 사항입니다.</font><p>
     <form name="bform" method="get">

	  <table border="0">
       <tr>
         <td width="5%" align="right"><img src="./img/bullet-02.gif"></td>
         <td width="15%"><font size="2 face="돋움"">글쓴이</font></td>
         <td width="80%">
         <input type="text" size="20" name="name"></td>
       </tr>
       <tr>
         <td align="right">&nbsp;</td>
         <td ><font size="2 face="돋움"">메일주소</font></td>
         <td>
          <input type="text" size="20" name="email"></td>
       </tr>
	   <tr>
         <td align="right"><img src="./img/bullet-02.gif"></td>
         <td><font size="2" face="돋움">제목</font></td>
         <td><input type="text" size="60" name="subject" ></td>
       </tr>
       <tr>
         <td align="right"><img src="./img/bullet-02.gif"></td>
         <td><font size="2" face="돋움">내용</font></td>
         <td><textarea wrap="physical" rows="10" name="contents" cols="60"></textarea></td>
       </tr>
	   <tr>
         <td align="right"><img src="./img/bullet-02.gif"></td>
         <td><font size="2" face="돋움">비밀번호</font></td>
          <td><input type="password" size="10" name="pass" ><font size="2" face="돋움">*.수정과 삭제시 꼭 입력하셔야 합니다.</font></td>
        </tr>
        <tr></tr>
		<tr>
          <td align="right">&nbsp;</td>
          <td><font size="2">&nbsp;</font></td>
          <td>
                     <a href="#" onClick="board_send()"><img src="./img/save.gif" border=0></a>&nbsp;&nbsp;&nbsp;
                     <a href="#"><img src="./img/cancle.gif" border=0></a>
                     <a href="#" onClick="board_list()"><input type="button" value="리스트"></a>
                     <input type= "hidden" name ="h" value="">
          </td>
        </tr>
      </table>
      </form>
    </td>
  </tr>
  </table>
  </body>
  </html>
